    ...
    <>
      <aside className="w-64 bg-white dark:bg-[#111111] border-r border-slate-200 dark:border-[#1f1f1f] flex flex-col hidden xl:flex shadow-sm relative z-50">
        ...
        <nav className="flex-1 px-4 space-y-2">
          <p className="px-4 text-[11px] font-bold text-slate-500 dark:text-[#888888] uppercase tracking-[0.3em] mb-6">MENU</p>
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-200 group focus-visible:ring-2 focus-visible:ring-[#0bd2b5] focus-visible:ring-offset-2 ${active ? 'bg-[#0bd2b5] text-slate-900 dark:text-black shadow-lg shadow-[#0bd2b5]/20' : 'text-slate-500 dark:text-[#888888] hover:bg-slate-50 dark:hover:bg-[#050505] hover:text-slate-900 dark:hover:text-white'}`}
              >
        ...
        {recentTrip && (
          <div className="px-4 pb-4">
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-[#050505] transition-all group text-left"
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-black italic uppercase tracking-tight text-slate-700 dark:text-[#aaa] truncate leading-none group-hover:text-[#0bd2b5] transition-colors">{recentTrip.name}</p>
                <p className="text-[10px] font-bold text-slate-400 dark:text-[#555] mt-0.5 truncate">{recentTrip.destination || recentTrip.status}</p>
              </div>
        ...
        <div className="px-4 py-5 mt-auto border-t border-slate-200 dark:border-[#1f1f1f] flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-[#0bd2b5] text-[#050505] flex items-center justify-center font-black text-xs italic shrink-0">{user?.initials || "AM"}</div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-black italic uppercase tracking-tight text-slate-900 dark:text-white truncate leading-none">{user?.name || "Ash Murray"}</p>
            <p className="text-[10px] font-bold text-slate-400 dark:text-[#666] uppercase tracking-wider leading-none mt-0.5 truncate">{user?.role || "Lead Designer"}</p>
          </div>
          <button aria-label="Sign out" onClick={() => setSignOutOpen(true)} className="h-8 w-8 rounded-lg flex items-center justify-center text-slate-400 dark:text-[#555] hover:text-red-500 hover:bg-red-500/10 transition-all shrink-0">
