  ...
  return (
    <div className={`flex items-center gap-3 px-3 py-2 ${collapsed ? "justify-center" : ""}`}>
      <Avatar className="h-8 w-8 rounded-xl shrink-0">
        <AvatarFallback className="rounded-xl bg-[#0bd2b5]/15 text-[#0bd2b5] text-[10px] font-black border border-[#0bd2b5]/20">
          {initials}
        </AvatarFallback>
      </Avatar>
      {!collapsed && (
                    ...
                    <SidebarMenuItem key={label}>
                      <SidebarMenuButton
                        ...
                        className={`
                          ...
                          ${active
                            ? "!bg-[#0bd2b5]/10 !text-[#0bd2b5] hover:!bg-[#0bd2b5]/10 hover:!text-[#0bd2b5]"
                      ...
                      >
                        {/* Active bar */}
                        {active && (
                          <span className="absolute left-0 inset-y-[7px] w-[3px] rounded-r-full bg-[#0bd2b5]" />
                        )}
                        <Icon className={`h-4 w-4 shrink-0 ${active ? "text-[#0bd2b5]" : ""}`} />
            ...
            <SidebarGroup>
              ...
              <SidebarGroupContent>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-bold text-[#aaa] truncate group-hover:text-[#0bd2b5] transition-colors">{recentTrip.name}</p>
                    <p className="text-[9px] text-[#444] mt-0.5 truncate">{recentTrip.destination || recentTrip.status}</p>
                  </div>
                  <ArrowUpRight className="h-3 w-3 text-[#444] group-hover:text-[#0bd2b5] transition-colors shrink-0" />
                </button>
              </SidebarGroupContent>
            </SidebarGroup>
          )}
