    ...
    <div ref={containerRef} className="relative">
      ...
      {showPanel && (
        <div
          ...
          className="absolute z-50 top-full mt-1 left-0 right-0 bg-card border border-border rounded-xl shadow-2xl overflow-hidden"
        >
          {status === "loading" && suggestions.length === 0 && (
            <p className="px-3 py-2.5 text-[11px] font-semibold text-muted-foreground">Searching...</p>
          )}
          {status === "empty" && (
            <p className="px-3 py-2.5 text-[11px] font-semibold text-muted-foreground">No places match. Try a city or a fuller address.</p>
          )}
          {status === "error" && (
          ...
          {suggestions.map((s, i) => (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 flex items-start gap-2.5 transition-colors border-b border-border last:border-0 ${i === activeIndex ? "bg-secondary" : "hover:bg-secondary"}`}
            >
              <MapPin className="h-3.5 w-3.5 text-brand mt-0.5 shrink-0" />
              <div className="min-w-0">
                <p className="text-xs font-bold text-foreground truncate">{s.text}</p>
                <p className="text-[10px] text-muted-foreground truncate">{s.place_name}</p>
              </div>
            </button>
          ))}
