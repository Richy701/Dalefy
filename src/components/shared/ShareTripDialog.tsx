...
export function ShareTripDialog({ open, onOpenChange, tripId, tripName }: ShareTripDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="!left-0 !top-0 !translate-x-0 !translate-y-0 w-full h-full max-w-none rounded-none border-0 bg-slate-100 dark:bg-[#050505] p-0 overflow-y-auto sm:!left-1/2 sm:!top-1/2 sm:!-translate-x-1/2 sm:!-translate-y-1/2 sm:w-[calc(100vw-2rem)] sm:max-w-md sm:h-auto sm:max-h-[calc(100vh-2rem)] sm:rounded-3xl sm:border sm:border-slate-200 sm:dark:border-[#1f1f1f] sm:overflow-hidden"
