...
export function DiningSearch({ onSelect, defaultLocation }: Props) {
  ...
  const inputCls =
    "w-full h-11 sm:h-9 px-3 rounded-lg bg-card border border-border text-base sm:text-sm font-semibold text-foreground placeholder:text-slate-400 dark:placeholder:text-[#555] focus:outline-none focus:border-brand transition-colors";
          ...
          {results.map((r, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-black text-foreground truncate">{r.name}</p>
                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                  ...
                  {r.reviews > 0 && <span className="text-[10px] text-muted-foreground">({r.reviews.toLocaleString()})</span>}
                  {r.priceTag && <span className="text-[10px] text-muted-foreground">{r.priceTag}</span>}
                  {r.cuisines?.length > 0 && <span className="text-[10px] text-muted-foreground">{r.cuisines.join(", ")}</span>}
                </div>
              </div>
